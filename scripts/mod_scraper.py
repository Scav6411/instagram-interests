...
def ensure_connection(conn):
    ...
    try:
        if conn.closed:
            print("[Warning] - Database connection closed. Reconnecting...")
            return connect_to_database()
    except Exception as e:
        print(f"[Error] - Failed to check or reconnect database connection: {e}")
        return None
    return conn
...
def scrape(use_proxy=False, proxy_info=None, posts_count=3):
    ...
    print(f"[Info] - Will scrape {posts_count} recent posts for each user")
    ...
    for user_id, instagram_id in pending_users:
        try:
            ...
            conn = ensure_connection(conn)  # Ensure connection before updating status
            if conn:
                update_scraping_status(conn, user_id, 'failed')
...
if __name__ == '__main__':
    ...
    posts_to_scrape = int(input("How many recent posts do you want to scrape per user? [default: 3]: ") or "3")
    
    scrape(use_proxy=use_proxy, proxy_info=proxy_info, posts_count=posts_to_scrape)
